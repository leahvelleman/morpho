...
from morpho.wrappers import PyniniWrapper
...
def transducers(draw):
    ...
    transducer = PyniniWrapper(pairs)
    return transducer

@composite
def fsts(draw):
    pairs = draw(transducertext())
    return PyniniWrapper(pairs)


@given(transducertext())
def test_wrapper_from_pairs(items):
    assume(items)
    wrapper = PyniniWrapper(items)
    assert wrapper

@given(transducertext())
def test_item_order_is_unimportant(items):
    assume(items)
    wrapper1 = PyniniWrapper(items)
    shuffle(items)
    wrapper2 = PyniniWrapper(items)
...
def test_concatenation(items1, items2):
    assume(items1)
    assume(items2)
    wrapper1 = PyniniWrapper(items1)
    wrapper2 = PyniniWrapper(items2)
...
def test_star(items, n):
    assume(items)
    wrapper = PyniniWrapper(items).star()
    for i in items:
...
def test_constructive_ops_mirror_set_ops(xs, ys):
    ...
    for op in ['__or__', '__and__', '__xor__', '__sub__']:
        fOp = getattr(PyniniWrapper, op)
        sOp = getattr(set, op)
        assert fOp(x, y) == PyniniWrapper(sOp(set(xs), set(ys)))
...
def test_constructive_operators_mirror_set_construction(xs, ys, zs):
    ...
    for op in ['union', 'intersection', 'difference']:
        ...
        assert (fOp(x, y, z) == 
                ...
                fOp(x, set(ys), z) ==
                fOp(x, set(ys), set(zs)) ==
                PyniniWrapper(sOp(set(xs), set(ys), set(zs))))

@given(lists(usabletext()), lists(usabletext()))
def test_concatenation(xs, ys):
    xset = PyniniWrapper(xs)
    yset = PyniniWrapper(ys)
    zset = xset + yset
    for x in xs:
        for y in ys:
            assert x + y in zset
